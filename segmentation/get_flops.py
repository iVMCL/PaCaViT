...
def inference(args: argparse.Namespace, logger: MMLogger) -> dict:
    ...
    if cfg.model.decode_head.type in ["MaskFormerHead", "Mask2FormerHead"]:
        ...
        raise NotImplementedError(
            "MaskFormer and Mask2Former are not " "supported yet."
        )
    outputs = get_model_complexity_info(
        model, input_shape, inputs=data["inputs"], show_table=False, show_arch=False
    )
    ...
    result["torchprofile_flops"] = _format_size(tp_flops)
