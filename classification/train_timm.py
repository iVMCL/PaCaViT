...
from models import *
from timm_custom.data import create_loader_v2
from timm_custom.utils import (
    ApexScalerV2,
    NativeScalerV2,
    update_summary_v2,
)
from timm_custom.optim import layerwise_lr_decay

where_am_I = pathlib.Path(__file__).parent.absolute()
_TARGET_DIR = os.path.expanduser(
    os.path.join(where_am_I, "../work_dirs/classification")
)
...
# << ------------- ivmcl
group = parser.add_argument_group("ivmcl modified settings")
# lr and grad. update
group.add_argument(
    "--auto-scale-warmup-min-lr",
    action="store_true",
    default=False,
    help="Auto-scale warmup and min lr with lr",
)
group.add_argument(
    "--grad-accumulation-steps",
    default=1,
    type=int,
    help="the number of steps accumulating gradients in training",
)
group.add_argument(
    "--use-deterministic",
    action="store_true",
    default=False,
    help="use deterministic settings",
)
group.add_argument(
    "--num-groups-in-layerwise-lr-decay",
    default=12,
    type=int,
...
group.add_argument(
    ...
    action="store_true",
    default=False,
    help="use 3 simple aug. methods from SSL",
)  # 3augment
group.add_argument(
    "--simple-random-crop",
    action="store_true",
    default=False,
    help="simple random crop from alexnet",
)  # simple random crop
# ivmcl ------------- >>
...
def main():
    ...
    args.output = args.output or _TARGET_DIR
    output_dir = os.path.join(args.output, exp_name)
    mmengine.utils.mkdir_or_exist(os.path.abspath(output_dir))

    completed_model_dir = os.path.join(args.output, "TrainingFinished")
    mmengine.utils.mkdir_or_exist(os.path.abspath(completed_model_dir))

    # zip models and classification
    shutil.make_archive(os.path.join(output_dir, "classification"), "zip", where_am_I)
    shutil.make_archive(
        os.path.join(output_dir, "models"),
        "zip",
        os.path.join(where_am_I, "../models"),
    )

    log_path = os.path.join(output_dir, "train_log.txt")
    has_log_path_before = os.path.exists(log_path)
    ...
    if args.channels_last:
        ...
        # << ------------- ivmcl
        if utils.is_primary(args):
            _logger.info(f"{dash_line} Channel Last Format is used.\n{dash_line}")
        # ivmcl ------------- >>
    ...
    if args.grad_accumulation_steps > 1:
        args.lr *= args.grad_accumulation_steps
        if args.auto_scale_warmup_min_lr:
            args.warmup_lr *= args.grad_accumulation_steps
            args.min_lr *= args.grad_accumulation_steps

    param_group_fn = None
    if args.layer_decay is not None:
        param_group_fn = partial(
            layerwise_lr_decay,
            num_groups=args.num_groups_in_layerwise_lr_decay,
            weight_decay=args.weight_decay,
            layer_decay=args.layer_decay,
            _logger=_logger if utils.is_primary(args) else None,
        )
    # ivmcl ------------- >>
    ...
    # << ------------- ivmcl
    if utils.is_primary(args):
        # model_stats = summary(model, data_config["input_size"][None, :], verbose=0)
        # summary_str = str(model_stats)
        # _logger.info("Model:\n" + dash_line + summary_str + "\n" + dash_line)

        args_text = yaml.safe_dump(args.__dict__, default_flow_style=False)
        _logger.info("Config:\n" + dash_line + args_text + "\n" + dash_line)

        _logger.info(f"Model {safe_model_name(args.model)} created.")
        args.mparams = sum([m.numel() for m in model.parameters()]) / 1e6
        _logger.info(f"{args.mparams:.3f}M Params")

        _logger.info(
            f"{dash_line} Grad_Accu_Steps={args.grad_accumulation_steps}\n{dash_line}"
        )

        # optimizer
        _logger.info(f"{dash_line} Optimizer: \n {optimizer.__repr__()}\n{dash_line}")
    # ivmcl ------------- >>
    ...
    if use_amp == "apex":
        ...
        loss_scaler = ApexScalerV2()  # ivmcl
        if utils.is_primary(args):
        ...
        if device.type == "cuda" and amp_dtype == torch.float16:
            ...
            loss_scaler = NativeScalerV2()  # ivmcl
        if utils.is_primary(args):
    ...
    # << ------------- ivmcl
    resume_epoch = None
    resume_ckpt = os.path.join(output_dir, "last.pth.tar")
    if not args.resume and os.path.exists(resume_ckpt):
    ...
    loader_train = create_loader_v2(
        ...
        # ivmcl
        use_simple_random_crop=args.simple_random_crop,
    ...
    try:
        if resume_epoch is not None:
            ...
            if utils.is_primary(args):
                _logger.info(
                    f"{dash_line} Resume training at epoch {resume_epoch}, and the validation results are: "
                    f"loss={resume_metrics['loss']}, top1={resume_metrics['top1']}, "
                    f"top5={resume_metrics['top5']}\n{dash_line}"
                )
        # ivmcl ------------- >>
        for epoch in range(start_epoch, num_epochs):
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                # eval_metrics = ema_eval_metrics # ivmcl

            if utils.is_primary(args):  # ivmcl
                if output_dir is not None:
                    lrs = [param_group["lr"] for param_group in optimizer.param_groups]
                    update_summary_v2(  # ivmcl
                        epoch,
                        train_metrics,
                        eval_metrics,
                ...
                if saver is not None:
                    ...
                    if ema_eval_metrics is not None:  # ivmcl
                        save_metric = max(save_metric, ema_eval_metrics[eval_metric])
            ...
            training_completed = epoch == num_epochs - 1
    ...
    if utils.is_primary(args):  # ivmcl
        if training_completed:
            this_model_dir = os.path.join(completed_model_dir, exp_name)
            mmengine.utils.mkdir_or_exist(os.path.abspath(this_model_dir))
            # backup this model
            filenames = [
                "model_best.pth.tar",
                "summary.csv",
            ...
            for f in filenames:
                ...
                dst = os.path.join(this_model_dir, f)
        ...
        if best_metric is not None:
            ...
            result_log = f"{timestamp} {exp_name} ({args.mparams:.3f}M Params): best metric {best_metric} (epoch {best_epoch})\n"

            filename = os.path.join(args.output, "all_classification_results.txt")
            with open(filename, mode="a") as cf:
                cf.write(result_log)

            if training_completed:
                filename = os.path.join(
                    completed_model_dir, "all_classification_results.txt"
                )
                with open(filename, mode="a") as cf:
                    cf.write(result_log)
    # ivmcl ------------- >>
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                # if utils.is_primary(args) and torch.any(torch.isnan(output)):
                #     _logger.info(
                #         f"Encountered NaN in testing at the {batch_idx}-th batch."
                #     )
